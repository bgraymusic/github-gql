...
class Query:
    ...
    def _fill_out_default_sub_fields(self, query_node: FieldNode, schema_node: GraphQLField) -> None:
        ...
        to_be_added = tuple(FieldNode(name=NameNode(value=x), directives=[], arguments=[])
                            for x in sorted(set(default_fields)) if x not in current_fields)
    ...
    def _is_paged_node(self, schema_node: GraphQLField) -> bool:
        ...
        fields = [x for x in self._get_schema_node_fields(schema_node)
                  if x in ['edges', 'nodes', 'pageInfo', 'totalCount']]
        ...
        if evidence_count == 8:
            ...
            raise ValueError(f'Some but not all evidence of paged node present:\n'
                             f'\tArgs: {args}\n'
                             f'\tFields: {fields}')
    ...
    def _fill_out_pagination_args(self, query_node: FieldNode) -> Direction:
        ...
        if first:
            ...
            new_arguments += (ArgumentNode(name=NameNode(value='first'),
                                           value=IntValueNode(value=self.default_page_size)),)
            direction = Query.Direction.FORWARD
        elif before:
            new_arguments += (ArgumentNode(name=NameNode(value='last'),
                                           value=IntValueNode(value=self.default_page_size)),)
            direction = Query.Direction.BACKWARD
        else:
            new_arguments += (ArgumentNode(name=NameNode(value='first'),
    ...
    def _fill_out_page_info(self, query_node: FieldNode, dir: Direction) -> None:
        ...
        if not pi:
            pi = FieldNode(name=NameNode(value='pageInfo'), directives=[], arguments=[],
                           selection_set=SelectionSetNode(selections=()))
            query_node.selection_set.selections += (pi,)
        new_selections = ()
        if dir == Query.Direction.FORWARD:
            pi.selection_set.selections = tuple(x for x in pi.selection_set.selections
                                                if x.name.value not in ['startCursor', 'hasPreviousPage'])
            ...
            pi.selection_set.selections = tuple(x for x in pi.selection_set.selections
                                                if x.name.value not in ['endCursor', 'hasNextPage'])
    ...
    def _edges_or_nodes_not_provided(self, query_node: FieldNode) -> bool:
        return (not query_node.selection_set or
                not next((x for x in query_node.selection_set.selections if isinstance(x, FieldNode)
                         and (x.name.value == 'edges' or x.name.value == 'nodes')), False))

    def _sub_in_edges_and_node(self, query_node: FieldNode) -> None:
        node = FieldNode(name=NameNode(value='node'), directives=[], arguments=[],
                         selection_set=query_node.selection_set)
        edges = FieldNode(name=NameNode(value='edges'), directives=[], arguments=[],
